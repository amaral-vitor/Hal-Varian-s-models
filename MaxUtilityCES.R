...
get_optimal_bundle <- function(delta, rho, p1, p2, w) {
  ...
  if (rho == 1) {
    ...
    f <- function(x1) {
      x2 <- (w - p1 * x1) / p2
      if (x2 < 0) return(-Inf)
      -ces_util(x1, x2, delta, rho)  # Negative because we want to maximize
    }
...
# Shiny UI
ui <- fluidPage(
  titlePanel("Utility Maximization - CES"),
  withMathJax(),
  ...
  sidebarLayout(
    sidebarPanel(
      sliderInput("delta", "Weight of Good 1 (δ)", min = 0.1, max = 0.9, value = 0.5, step = 0.05),
      ...
      br(), br(),
      numericInput("w", "Income (w)", value = 100),
...
server <- function(input, output, session) {
  ...
  output$max_plot <- renderPlotly({
    ...
    x1_vals <- seq(0, w / p1 * 1.2, length.out = 100)
    x2_vals <- seq(0, w / p2 * 1.2, length.out = 100)
    ...
    plot %>%
      layout(
        title = list(text = "Utility Maximization", x = 0.2),
  ...
  output$info <- renderText({
    ...
    if (is.list(opt)) {
      paste0("Optimum: x1 = ", round(opt$x1, 2), ", x2 = ", round(opt$x2, 2))
    } else {
      "Every point along the budget line is optimal."
    }
  })
}
